...
- **Guardraile Sentry (nie usuwać):** `sendDefaultPii: false`, `beforeBreadcrumb` (drop `console`, `touch` bez `message`/`data`, `navigation` bez query stringa), `beforeSend` (`delete event.user` + twardy limit `MAX_EVENTS_PER_SESSION = 25`), `beforeSendTransaction` (`delete event.user` + osobny `MAX_TRANSACTIONS_PER_SESSION = 10`, bo `beforeSend` nie widzi transakcji), `no-console` w ESLint, serwerowa reguła scrubbingu `$user.geo.**`. Zmiana konfiguracji Sentry pociąga politykę prywatności i formularz Data Safety w Play Console. Uzasadnienie: `docs/DECISIONS.md`.
