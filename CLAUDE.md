...
- AsyncStorage (tylko flagi UI, np. onboarding — NIE dane domenowe). Jedyny wyjątek: `crashReportingEnabled` + `crashReportingConsentMeta` (`utils/crashReporting.ts`) — czytane przed inicjalizacją bazy i muszą przeżyć reset bazy. Uzasadnienie: `docs/DECISIONS.md`.
...
- **Sentry = init warunkowy (opt-in):** `bootstrapCrashReporting()` w `app/_layout.tsx` woła `Sentry.init` dopiero po odczycie flagi zgody — bez jawnej zgody (i zawsze w `__DEV__`) klient **nie powstaje w ogóle**. NIE zastępuj tego `enabled: false`. Ścieżka bez klienta jest **domyślna**, więc: **każde** nowe wywołanie API Sentry musi być odporne na brak klienta — tylko metody z twardym guardem (`captureException`, `captureMessage`, `getClient()?.`). Uzasadnienie: `docs/DECISIONS.md`.
- **Zgoda na crash reporting:** flaga `crashReportingEnabled` w AsyncStorage zostaje prostym `'true'`/`'false'` (leci w ścieżce bootu — nie rozbudowywać); default i fallback przy błędzie odczytu = `false`. Metadane zgody w osobnym kluczu `crashReportingConsentMeta`, zapisywane fire-and-forget. Zbierane slajdem w onboardingu i przełącznikiem w profilu — zmiana działa dopiero od kolejnego startu aplikacji. Uzasadnienie: `docs/DECISIONS.md`.
- **Guardraile Sentry (nie usuwać):** `sendDefaultPii: false`, `beforeBreadcrumb` (drop `console`, `touch` bez `message`/`data`, `navigation` bez query stringa, `network.event` przycięte do `action`/`network_type`), `beforeSend` (`delete event.user` + `stripVpnFlag` + twardy limit `MAX_ERRORS_PER_SESSION = 25`), `beforeSendTransaction` (`delete event.user` + `stripVpnFlag` + osobny `MAX_TRANSACTIONS_PER_SESSION = 10`, bo `beforeSend` nie widzi transakcji), `no-console` w ESLint, serwerowe reguły scrubbingu `$user.geo.**` i `$breadcrumb.data.{signal_strength,download_bandwidth,upload_bandwidth,vpn_active}`. **Natywne breadcrumby Androida (`network.event`, `device.event`, `app.lifecycle`) nie przechodzą przez `beforeBreadcrumb`** — czyści je dopiero scrubbing serwerowy plus `stripVpnFlag` na gotowym evencie; audyt guardraili wyłącznie przez lekturę kodu JS jest niepełny. Konfiguracja projektu Sentry jest częścią guardraili, nie ustawieniem kosmetycznym. Zmiana konfiguracji Sentry pociąga politykę prywatności i formularz Data Safety w Play Console. Uzasadnienie: `docs/DECISIONS.md`.
